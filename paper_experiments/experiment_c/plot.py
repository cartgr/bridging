"""
Experiment C: MDS ridgeline plots for real Polis data.

Loads JSON from run.py, produces ridgeline plots with 3 metric columns.
Top 5 comments by PD, with wrapped comment text.
"""
...
def escape_latex(text):
    ...
    # Remove non-ASCII characters (e.g., Chinese text that LaTeX can't render)
...
def plot_ridgeline(data: dict, output_path: Path, top_n: int = 5):
    """Create MDS ridgeline plot for one Polis dataset."""
    ...
    # Top N by PD (exclude NaN)
    valid = ~np.isnan(pd_scores)
    valid_indices = np.where(valid)[0]
    sorted_valid = valid_indices[np.argsort(pd_scores[valid_indices])[::-1]]
    top_indices = sorted_valid[:top_n]
    ...
    # Pre-compute wrapped text and row heights for each comment
    wrap_width = 45
    min_row_height = 0.7
    line_height = 0.12  # Height per line of text
    wrapped_texts = []
    row_heights = []
    for idx in top_indices:
    ...
    # Figure setup
    fig_width = 7.5  # Slightly wider for comment text
    fig_height = 0.6 + total_height
    fig, ax = plt.subplots(figsize=(fig_width, fig_height))

    # Colormap (blue = left/one group, red = right/other group)
    ...
    for row_idx, item_idx in enumerate(top_indices):
        ...
        # Comment text (left side) - use pre-wrapped text
        wrapped = wrapped_texts[row_idx]
        ax.text(
            pos_min - x_margin - 0.03, y_base + row_height * 0.4,
            wrapped,
            fontsize=6,
            ha="right",
            va="center",
    ...
    # Axes limits
    ax.set_xlim(pos_min - x_margin - 0.55, pos_max + x_margin + 1.3)
    ...
    ax.text(pos_min - x_margin - 0.28, y_header, "Comment",
...
def main():
    ...
    for file_id, data in all_results.items():
        ...
        output_path = plots_dir / f"experiment_c_{file_id}.png"
